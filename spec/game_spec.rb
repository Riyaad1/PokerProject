...
RSpec.describe Game do
  ...
  describe '.discard_choice' do #There might be a weird chance were the decks become same due to the shuffle
    ...
    it 'discards 1 card' do
      allow_any_instance_of(Game).to receive(:gets).and_return('y', 1, 0)
    ...
    it 'discards 2 card' do
      allow_any_instance_of(Game).to receive(:gets).and_return('y', 2, 0)
      new_game.masterdeck.first_turn = false
      x = temp_player.hand.player_deck.dup
      new_game.discard_choice(temp_player)
      expect(temp_player.hand.player_deck).to_not eq(x)
    ...
    it 'discards 3 card' do
      allow_any_instance_of(Game).to receive(:gets).and_return('y', 3, 0)
      new_game.masterdeck.first_turn = false
      x = temp_player.hand.player_deck.dup
      new_game.discard_choice(temp_player)
      expect(temp_player.hand.player_deck).to_not eq(x)
  ...
  describe '.determine_win' do
    ...
    let(:player3) { Player.new([Card.new('Hearts', '10'), Card.new('Hearts', 'Jack'), Card.new('Hearts', 'Queen'), Card.new('Hearts', 'King'), Card.new('Hearts', 'Ace')], 1000)}
    it 'determines a win' do
      new_game.players << player1
      new_game.players << player2
      expect(new_game.determine_win(player1)).to eq('Current Player Wins')
    end
    it 'determines a loss' do
      new_game.players << player1
      new_game.players << player2
      expect(new_game.determine_win(player2)).to eq('Current Player Loses')
    end
    it 'determines a tie' do
      new_game.players << player1
      new_game.players << player3
      expect(new_game.determine_win(player1)).to eq('Players Tie')
    end
  end
end
